...
#define APP_DIR_NAME		L"win32CrashReport"
...
//获取75*75尺寸游戏图标路径
inline wstring GetGamesIco100Path()
{
	wstring strPath = GetAppDataPath() + L"Icons\\100";
	if (!PathFileExists(strPath.c_str()))
		SHCreateDirectory(NULL, strPath.c_str());
	return strPath;
}
...
//获取20*20尺寸图标路径
inline wstring GetGamesIco20Path()
{
	wstring strPath = GetAppDataPath() + L"Icons\\20";
	if (!PathFileExists(strPath.c_str()))
		SHCreateDirectory(NULL, strPath.c_str());
	return strPath;
}
